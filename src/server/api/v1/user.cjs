...
module.exports = (app) => {
  ...
  app.put('/v1/user', async (req, res) => {
    ...
    try {
      ...
      if (
        data.primary_email &&
        data.primary_email !== req.session.user.primary_email
      ) {
        const existingEmail = await app.models.User.findOne({
          primary_email: data.primary_email,
        });
        if (existingEmail) {
      ...
      try {
        const updatedUser = await app.models.User.findOneAndUpdate(
          query,
          { $set: data },
          { new: true },
        );
        ...
        console.error(
          `User.update: User not found or update failed for ${req.session.user.id}`,
        );
